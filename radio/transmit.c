...
void lora_init()
{
	sx1276_init(local_rx_handler, lora_rx_handler, timeout_handler, crc_err_handler);
	sx1276_sleep();
...
void lora_send_pkt(struct Packet_t *pkt)
{
	...
	debug_uart_sendstr("Packet sent \n\r");
...
/*void lora_rx_handler(void) //outsourced, code left here as example that is known to work RK
{
	struct packet_t received;	// Buffer for data rx
	debug_uart_sendstr("New Packet \n\r");
...
//process the packet
		...
		led_blink();
		debug_uart_sendstr(received.data);
		debug_uart_newline();
